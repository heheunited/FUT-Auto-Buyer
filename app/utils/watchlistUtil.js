            ...
            async function (t, watchResponse) {
              ...
              if (isAutoBuyerActive && bidPrice) {
                let outBidItems = watchResponse.data.items.filter(function (item) {
                  ...
                  let expireTimeLessThan = buyerSetting['idAbBidExpiresLessThanSeconds']
                      ? auction.expires <= buyerSetting['idAbBidExpiresLessThanSeconds'] > 0
                      : true;
                ...
                for (var i = 0; i < outBidItems.length; i++) {
                  ...
                  if (
                    ...
                    writeToLog(
                        `[???] Bid dont make excepted profit percent: ${expectedProfitPercent}%. Player: ${currentItem._staticData.name}. Bid: ${checkPrice}. FB price: ${getFutBinPlayerPrice(currentItem.definitionId)}`,
                        idProgressAutobuyer
                    );
                    continue;
                  }
